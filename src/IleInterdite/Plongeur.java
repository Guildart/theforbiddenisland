...
public class Plongeur extends Player {
    ...
    public ArrayList<Zone> zonesReachable(){
        ...
        zonesSafe = getZoneSousMarines(zone,0,zonesSafe);
       /*if (pos.y-1>=0)
    ...
    public ArrayList<Zone> getZoneSousMarines(Zone z, int depth, ArrayList<Zone> chemin ){
        ...
        if (chemin.size() ==0) {
            ...
            if(pos.x-1>=0){
                if(zones[pos.x-1][pos.y].isSafe())
                    zonesSafe.add(zones[pos.x-1][pos.y]);
                else if (!chemin.contains(zones[pos.x-1][pos.y])){
                    chemin.add(zones[pos.x][pos.y+1]);
                    ArrayList<Zone> a = getZoneSousMarines(zones[pos.x-1][pos.y],depth+1, chemin);
                   /* if (a.size()>0)
                        for (Zone zone1 : a)
            ...
            if(pos.y-1>=0){
                if(zones[pos.x][pos.y-1].isSafe())
                    zonesSafe.add(zones[pos.x][pos.y-1]);
                else if (!chemin.contains(zones[pos.x][pos.y-1])){
                    chemin.add(zones[pos.x-1][pos.y]);
                    ArrayList<Zone> a = getZoneSousMarines(zones[pos.x][pos.y-1],depth+1,chemin);
                   /* if (a.size() > 0)
                        for (Zone zone1 : a)
                            if (!zonesSafe.contains(z))
                                zonesSafe.add(zone1);*/
